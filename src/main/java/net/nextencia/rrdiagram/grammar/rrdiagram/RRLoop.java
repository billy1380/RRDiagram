...
public class RRLoop extends RRElement {
	...
	public RRLoop (RRElement rrElement, RRElement loopElement,
			int minRepetitionCount, Integer maxRepetitionCount) {
		this.rrElement = rrElement;
		this.loopElement = loopElement;
		if (minRepetitionCount < 0) { throw new IllegalArgumentException(
				"Minimum repetition must be positive!"); }
		if (maxRepetitionCount != null
				&& maxRepetitionCount < minRepetitionCount) { throw new IllegalArgumentException(
						"Maximum repetition must not be smaller than minimum!"); }
	...
	protected void computeLayoutInfo (RRDiagramToSVG rrDiagramToSVG) {
		...
		if (minRepetitionCount > 0 || maxRepetitionCount != null) {
			cardinalitiesText = minRepetitionCount + ".."
					+ (maxRepetitionCount == null ? "N" : maxRepetitionCount);
			Font font = rrDiagramToSVG.getLoopFont();
			fontYOffset = Math
					.round(font.getLineMetrics(cardinalitiesText).descent);
			Rectangle2D stringBounds = font.getStringBounds(cardinalitiesText);
			cardinalitiesWidth = (int) Math.round(stringBounds.width) + 2;
	...
	@Override
	protected void toSVG (RRDiagramToSVG rrDiagramToSVG, int xOffset,
			int yOffset, SvgContent svgContent) {
		LayoutInfo layoutInfo1 = rrElement.getLayoutInfo();
		...
		svgContent.addLineConnector(x1 - 10, y2,
				x1 + 10 + (maxWidth - width1) / 2, y2);
		...
		if (cardinalitiesText != null) {
			String cssClass = svgContent.getDefinedCSSClass(
					RRDiagram.CSS_LOOP_CARDINALITIES_TEXT_CLASS);
			if (cssClass == null) {
				Font loopFont = rrDiagramToSVG.getLoopFont();
				String loopTextColor = Utils
						.convertColorToHtml(rrDiagramToSVG.getLoopTextColor());
				cssClass = svgContent.setCSSClass(
						RRDiagram.CSS_LOOP_CARDINALITIES_TEXT_CLASS,
						"fill:" + loopTextColor + ";"
								+ Utils.convertFontToCss(loopFont));
			}
			svgContent.addElement("<text class=\"" + cssClass + "\" x=\""
					+ (x2 - cardinalitiesWidth) + "\" y=\""
					+ (y2 - fontYOffset - 5) + "\">"
					+ Utils.escapeXML(cardinalitiesText) + "</text>");
		}
		rrElement.toSVG(rrDiagramToSVG, xOffset + 20 + (maxWidth - width1) / 2,
				yOffset2, svgContent);
		svgContent.addLineConnector(
				x2 - cardinalitiesWidth - 10 - (maxWidth - width1) / 2, y2,
				xOffset + layoutInfo.getWidth(), y2);
	}
